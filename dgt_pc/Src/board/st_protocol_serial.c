...
#define  LOG_MODULE_NAME     "[st_protocol_serial]"

int st_protocol_serial_init(uint8_t port,uint32_t bauds,uint8_t data_bit,uint8_t stop_bit);
int st_protocol_serial_deinit(uint8_t port);
void st_protocol_serial_enable_txe_int();
void st_protocol_serial_disable_txe_int();
void st_protocol_serial_enable_rxne_int();
void st_protocol_serial_disable_rxne_int();


extern int st_protocol_serial_handle;

serial_hal_driver_t protocol_serial_driver={
.init=st_protocol_serial_init,
.deinit=st_protocol_serial_deinit,
.enable_txe_int=st_protocol_serial_enable_txe_int,
.disable_txe_int=st_protocol_serial_disable_txe_int,
.enable_rxne_int=st_protocol_serial_enable_rxne_int,
.disable_rxne_int=st_protocol_serial_disable_rxne_int
};

extern UART_HandleTypeDef huart0;
UART_HandleTypeDef *st_serial = &huart0;
...
void st_protocol_serial_isr(void)
{
  ...
  isr_serial_put_byte_from_recv(st_protocol_serial_handle,recv_byte);
  }
  ...
  if((tmp_flag != RESET) && (tmp_it_source != RESET))
  {
    result =isr_serial_get_byte_to_send(st_protocol_serial_handle,&send_byte);
    if(result == 1)
    {
